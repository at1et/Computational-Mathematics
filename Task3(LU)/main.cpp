...
const int n = 6;  // Размерность матрицы и вектора
...
void luDecomposition(double mat[][n], double L[][n], double U[][n]) {
    for (int i = 0; i < n; i++) {
        ...
        for (int k = i; k < n; k++) {
            if (i == k)
                L[i][i] = 1; // Установка диагональных элементов L равными 1
            else {
                double sum = 0;
                for (int j = 0; j < i; j++)
                    sum += (L[k][j] * U[j][i]);
...
int main() {
    ...
    double B[n] = { 28, 29, 21, 24, 9 };  // Вектор свободных членов
